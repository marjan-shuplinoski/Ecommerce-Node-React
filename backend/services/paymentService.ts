 ...
 */
export const createPayment = async (paymentData: PaymentType, user: { id: string, role: string }) => {
 ...
 */
export const getPayments = async (user: { role: string }) => {
  if (user.role !== 'admin') {
 ...
 */
export const getPaymentById = async (id: string, user: { id: string, role: string }) => {
 ...
 * @throws Error if not found or unauthorized
 */
export const updatePayment = async (id: string, update: Partial<PaymentType>, user: { role: string }) => {
  if (user.role !== 'admin') {
    throw new Error('Forbidden: Admins only');
  }
 ...
 * @throws Error if not found or unauthorized
 */
export const deletePayment = async (id: string, user: { role: string }) => {
  if (user.role !== 'admin') {
    throw new Error('Forbidden: Admins only');
  }
