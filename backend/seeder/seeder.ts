...
const MONGO_URI: string = process.env.MONGO_URI || 'mongodb://localhost:27017/ecommerce_dev';
...
async function seed(): Promise<void> {
  ...
  if (Array.isArray(user1.orders)) {
    user1.orders.push(order1._id as any);
  } else {
    user1.orders = [order1._id as any];
  }
  await user1.save();

  // Payment
  const payment1 = await Payment.create({ order: order1._id, amount: 719, method: 'card', status: 'paid', provider: 'Stripe', paidAt: new Date() });
  order1.payment = payment1._id as any;
  await order1.save();
  ...
  order1.delivery = delivery1._id as any;
  await order1.save();

  // Mark coupon as used
  coupon10.usedCount = 1;
  if (Array.isArray(coupon10.usersUsed)) {
    coupon10.usersUsed.push(user1._id as any);
  } else {
    coupon10.usersUsed = [user1._id as any];
  }
  await coupon10.save();
