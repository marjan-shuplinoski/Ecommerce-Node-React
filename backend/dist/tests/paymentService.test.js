...
describe('paymentService', () => {
    ...
    it('admin can create payment', async () => {
        const payment = await paymentService.createPayment(paymentData, adminUser);
        expect(payment).toHaveProperty('_id');
        paymentId = payment._id.toString();
    });
    it('user can create own payment', async () => {
        const payment = await paymentService.createPayment({ ...paymentData, user: normalUserId }, normalUser);
        expect(payment).toHaveProperty('_id');
    });
    it('user cannot create payment for another user', async () => {
        const otherUserId = new mongoose.Types.ObjectId();
        await expect(paymentService.createPayment({ ...paymentData, user: otherUserId }, normalUser)).rejects.toThrow('Forbidden');
    });
    ...
    it('user cannot delete payment', async () => {
        const newPayment = await paymentService.createPayment({ ...paymentData, transactionId: 'txn_new', user: normalUserId }, normalUser);
        await expect(paymentService.deletePayment(newPayment._id.toString(), normalUser)).rejects.toThrow('Forbidden');
    });
});
